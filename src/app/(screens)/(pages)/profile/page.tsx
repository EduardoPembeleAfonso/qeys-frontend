...
const usersTypes = ["Proprietário", "Inquilino"];
export default function Page() {
  ...
  const [openEditUserProfile, setOpenEditUserProfile] =
  ...
  const bankAccounts = _bankAccounts;
  ...
  const onOpenEditUserProfile = () => {
    if (user?.name || user?.email || user?.image) {
      ...
      setEmail(user?.email);
      setUserTypes(
        user?.type === USER_TYPES.TENANT ? "Inquilino" : "Proprietário"
      );
  ...
  async function handleEditUserProfile(e: FormEvent<HTMLFormElement>) {
    ...
    if (user && user?.id) {
      ...
      await updateUser(user?.id, image, name, email, userTypes);
  ...
  return (
    <div className="flex justify-between h-full w-full">
              ...
              {user ? (
                <>
                  ...
                  <div className="relative -top-8 -right-8 bg-secondaryColor w-8 h-8 p-1 rounded-full border border-borderColor hover:bg-borderColor">
                    <button
                      onClick={() => onOpenEditUserProfile()}
                      className="w-full h-full"
                    >
                      <Edit3Icon color="#0364cc" className="w-full h-full" />
                    </button>
                  </div>
                  <h2 className="mt-5 font-MontserratSemiBold text-2xl text-blackColor">
                    {user?.name}
                  </h2>
                  <div className="mt-4 flex items-center justify-start gap-2">
                    <UserSquare color="#0364cc" className="w-[16px] h-[16px]" />
                    <span className="text-blackColor text-lg font-Poppins font-normal">
                  ...
                  <div className="mt-2 flex items-center justify-start gap-2">
                    <MailIcon color="#0364cc" className="w-[16px] h-[16px]" />
                    <span className="text-blackColor text-lg font-Poppins font-normal">
                      {user?.email}
                    </span>
                  </div>
              ...
              <div className="h-full w-full flex flex-col lg:flex-row items-center lg:justify-between gap-1 lg:gap-4">
                <div className="flex items-center mt-5 gap-5 px-5 w-full md:w-[325px] h-[70px] border border-borderColor rounded-[20px]">
                  ...
                  <div className="flex flex-col items-start">
                    <span className="text-lg text-textColor font-semibold font-MontserratSemiBold">
                      {
                        data?.filter((i: IProperties) => i.userId === authorId)
                          ?.length
                      }
                    </span>
                ...
                <div className="flex items-center mt-5 gap-5 px-5 w-full md:w-[325px] h-[70px] border border-borderColor rounded-[20px]">
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    <span className="text-sm text-[#9D9D9D] font-normal font-Montserrat">
                      Total de imoveis alugados/comprados
                    </span>
                  </div>
                </div>
              </div>
              <div className="w-full flex flex-col items-center justify-center md:items-start md:justify-start">
                <h4 className="text-xl font-Poppins mt-3 text-textColor font-semibold md:mt-2">
                  Top 2 imoveis mais bem avaliados
                </h4>
                <div className="mt-5 w-full h-auto md:h-[220px] flex flex-col items-center justify-center rounded-lg md:grid md:grid-cols-2">
                  {isLoading
                    ...
                    : data
                        ?.filter((i: IProperties) => i.userId === user?.id)
                        .map((item: IProperties) => (
                          <div key={item.id}>
                            <div className="cursor-pointer bg-white flex flex-col items-center justify-center w-[268px] h-[220px] border border-borderColor rounded-[5px]">
                              ...
                              <h3 className="mt-[6px] w-[230px] text-textColor text-base font-semibold font-MontserratSemiBold">
          ...
          <div className="w-full my-5">
            {bankAccounts.isLoading ? (
              ...
              <Table>
                ...
                <TableBody>
                  {bankAccounts.data
                    ?.filter((i: IBankAccounts) => i.userId === user?.id)
                    ?.map((item) => (
      ...
      <Modal
        ...
        onClose={onCloseModalEditUserProfile}
        center
      >
        <form
          ...
          <div className="grid gap-2 py-2 px-5 mt-3">
            ...
            <div className="flex flex-col gap-4">
              ...
              <CustomSelect
                options={usersTypes}
                placeholder={`${
                  userTypes ? userTypes : "Selecione o tipo de conta"
                } `}
